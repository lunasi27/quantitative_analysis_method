    ...
    def periodCheck(self, stocks):
        ...
        for stock in stocks:
            ...
            if on_board_days > one_month:
                selected_stocks.append(stock)
        self.logger.info('过滤新股后，剩余标的%s个' % len(selected_stocks))
        return selected_stocks
...
class SelBuyButtom(SelBase):
    ...
    def rsiCheck(self, stocks):
        ...
        for stock in stocks:
            ...
            if rsi_data < rsi_threshold:
                selected_stocks.append(stock)
                self.logger.info('%s股票RSI值=%s低于下限30' % (stock, rsi_data))
        return selected_stocks
    ...
    def rsiCheckAdv(self, stocks):
        ...
        for stock in stocks:
            ...
            if rsi_data < rsi_threshold:
                selected_stocks.append(stock)
                self.logger.info('%s股票RSI值=%s低于下限20' % (stock, rsi_data))
        return selected_stocks
    ...
    def fallStopCheck(self, stocks):
        ...
        for stock in stocks:
            ...
            if not (Utility.isContinueFallStop(close_prices, min_co_present=2)
                ...
                self.logger.info('%s股票一月内存在连续跌停，剔除')
        return selected_stocks
...
class SelChaseRise(SelBase):
    ...
    def search(self, stocks):
        ...
        self.logger.info('符合追涨条件股票%s个' % len(self.selected_stocks))
        # Advance condiction
        return self.selected_stocks
    ...
    def periodCheck(self, stocks):
        ...
        for stock in stocks:
            ...
            if on_board_days < one_year:
                ...
                selected_stocks.append(stock)
        self.logger.info('过滤掉上市小于一个月或上市大于8个月且小于一年的股票，剩余%s只股票' % len(selected_stocks))
        return selected_stocks
        ...
        for stock in stocks:
            ...
            if Utility.isRiseStopNow(close_prices) \
               and not Utility.isContinueRiseStop(close_prices, max_co_present=2):
                    selected_stocks.append(stock)
                    self.logger.info('选出当日涨停且不连续涨停的股票%s' % stock)
        return selected_stocks
    ...
    def riseRateCheck(self, stocks):
        ...
        for stock in stocks:
            ...
            if rise_rate < 0.25:
                selected_stocks.append(stock)
                self.logger.info('选出过去10个交易日内，涨幅%s的股票，涨幅小于25%%' % rise_rate*100)
        return selected_stocks
        ...
        for stock in stocks:
            ...
            if close_prices.max() == close_prices[-1]:
                if open_prices[-1] <= high_prices[-2] and open_prices[-1] >= low_prices[-2]:
                    selected_stocks.append(stock)
                    self.logger.info('选出收盘价创新高且未出现跳空缺口的股票%s' % stock)
        return selected_stocks
    ...
    def rsiCheck(self, stocks):
        ...
        for stock in stocks:
            ...
            if rsi_data.min() < 50:
                selected_stocks.append(stock)
                self.logger.info('选出前十个交易日内，RSI指标=%s，且小于30的股票%s' % (rsi_data[-1], stock))
        return selected_stocks
    ...
    def breakAvgsAdv(self, stocks):
        ...
        for stock in stocks:
            close_prices = history_bars(stock, self.load_period, '1d', 'close')
            avg20 = talib.MA(close_prices, timeperiod=check_period,matype=0)
            avg30 = talib.MA(close_prices, timeperiod=check_period,matype=0)
            avg60 = talib.MA(close_prices, timeperiod=check_period,matype=0)
            # 选出三天中期均线中离昨天收盘价最近的数据（且中期均线必须小于收盘价）。
            if close_prices[-1] > avg60[-1] or close_prices[-1] > avg30[-1] or close_prices[-1] > avg20[-1]:
                selected_stocks.append(stock)
                self.logger.info('选出突破中期均线的股票%s' % stock)
        return selected_stocks
...
class SelSeekBoard(SelBase):
    ...
    def search(self, stocks):
        ...
        self.logger.info('符合打板条件股票%s个' % len(self.selected_stocks))
        return self.selected_stocks
        ...
        for stock in stocks:
            ...
            if Utility.isContinueRiseStop(close_prices, max_co_present=3) \
                and not Utility.isContinueRiseStop(close_prices, max_co_present=4):
                    selected_stocks.append(stock)
                    self.logger.info('选出三连板的股票%s' % (stock))
        return selected_stocks
    ...
    def tunoverRateCheck(self, stocks):
        ...
        selected_stocks = []
        for stock in stocks:
            vols = history_bars(stock, check_period, '1d', 'total_turnover')
            if Utility.isabsoluteIncr(vols):
                selected_stocks.append(stock)
                self.logger.info('选出成交量逐渐放大的股票%s' % (stock))
        return selected_stocks
